...
public class GL15 extends GL14C {
    ...
    public static void glBufferData(int target, short[] data, int usage) { if (Gdx.gl20 != null) { java.nio.ByteBuffer bb = java.nio.ByteBuffer.allocateDirect(data.length * 2).order(java.nio.ByteOrder.nativeOrder()); bb.asShortBuffer().put(data); bb.rewind(); Gdx.gl20.glBufferData(target, data.length * 2, bb, usage); } }
